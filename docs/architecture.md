...
pub trait StorageBackend: Send + Sync {
    ...
    fn scan_prefix(&self, collection: &str, prefix: &[u8]) -> Result<Box<dyn Iterator<Item = (Vec<u8>, Vec<u8>)>>>;
...
See [storage-backends.md](storage-backends.md) for implementation details.
...
- **SQLite**: Uses FTS5 extension (built-in)
- **Sled/LMDB**: Uses tantivy (embedded Lucene-like engine)
...
See [storage-backends.md](storage-backends.md) for implementation guide.
